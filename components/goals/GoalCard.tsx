...
export function GoalCard({ goal, onClick, compact = false }: GoalCardProps) {
  ...
  return (
    ...
    >
          ...
          {!compact && (
            <>
              <button onClick={handleComplete} title={goal.status === "completed" ? "Mark active" : "Mark complete"}
                style={{ background: "none", border: "none", cursor: "pointer", color: goal.status === "completed" ? "#16a34a" : "#9ca3af", padding: 4, borderRadius: 6, display: "flex", transition: "color 0.15s" }}>
                <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M20 6L9 17l-5-5" />
                </svg>
              </button>
              <button onClick={handleDelete}
                style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af", padding: 4, borderRadius: 6, display: "flex", transition: "color 0.15s" }}
      ...
      {/* Title */}
      <h3 style={{ fontSize: compact ? 14 : 15, fontWeight: 700, marginBottom: compact ? 6 : 8, lineHeight: 1.3, color: "#1a1a2e" }}>{goal.title}</h3>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        {deadline ? (
          ...
          <span style={{ fontSize: 12, color: "#9ca3af" }}>No deadline</span>
        )}
